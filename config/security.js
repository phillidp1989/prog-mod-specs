...
function validateEnvironmentVariables() {
  ...
  // Optional email service variables - warn if missing but don't fail
  const emailVars = ['GMAIL_USER', 'GMAIL_APP_PASSWORD', 'NOTIFICATION_EMAIL'];
  const missingEmail = [];

  emailVars.forEach(varName => {
    if (!process.env[varName]) {
      missingEmail.push(varName);
    }
  });

  if (missingEmail.length > 0) {
    console.warn('⚠ Gmail SMTP variables not configured:', missingEmail.join(', '));
    console.warn('⚠ Email notifications will be disabled');
    console.warn('⚠ To enable emails, add Gmail credentials to .env file');
  } else {
    console.log('✓ Gmail SMTP variables present');
  }
}
